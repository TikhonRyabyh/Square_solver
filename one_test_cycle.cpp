...
int one_test_cycle( data_list_st* now_member, int all_tests ) {
	...
	/*if( ( !( isfinite( x1 ) ) && isfinite( now_member->right_x1 ) ) || ( isfinite( x1 ) && !( isfinite( now_member->right_x1 ) ) ) 
	|| ( !( isfinite( x2 ) ) && isfinite( now_member->right_x2 ) ) || ( isfinite( x2 ) && !( isfinite( now_member->right_x2 )  ) ) ) { 
	...
	if( ( !( isfinite( x1 ) ) && !( isfinite( now_member->right_x1 ) )  && n == now_member->right_n && return_ == now_member->right_return ) 
	|| ( !( isfinite( x2 ) ) && !( isfinite( now_member->right_x2 ) ) && n == now_member->right_n && return_ == now_member->right_return ) ) { 
